...
def get_setting(name, default=None):
    settings = sublime.load_settings('SublimeProfiles.sublime-settings')
    return settings.get(name) or default

def set_setting(name, value):
    settings = sublime.load_settings('SublimeProfiles.sublime-settings')
    settings.set(name, value)
    sublime.save_settings('SublimeProfiles.sublime-settings')
...
def create_profile(profile_name):
    ...
    if profile_name in [x['name'] for x in data['profiles']]:
        sublime.error_message('SublimeProfiles: There is already a profile named: "{}"'.format(profile_name))
        return
    ...
    print('SublimeProfiles: Created profile "{}"'.format(profile_name))
...
def delete_profile(profile_name):
    ...
    if profile_name not in [x['name'] for x in data['profiles']]:
        sublime.error_message('SublimeProfiles: There is no profile named: "{}"'.format(profile_name))
        return
    if profile_name == 'default':
        sublime.error_message('SublimeProfiles: Cannot delete default package!')
        return
    ...
    if get_setting('active_profile', None) == profile_name:
        print('SublimeProfiles: Deleting active profile! Switching to default profile')
    ...
    print('SublimeProfiles: Deleted profile "{}"'.format(profile_name))
...
def sync_active_profile():
    ...
    print('SublimeProfiles: Syncing profile "{}"'.format(profile_name))
...
def switch_profile(profile_name):
    ...
    if get_setting('active_profile', None) == profile_name:
        print('SublimeProfiles: Profile "{}" is already active!'.format(profile_name))
        return
...
def upload_settings_to(profile_name):
    ...
    if meta is None or meta['md5Checksum'] != md5_file(filepath):
        print('SublimeProfiles: Updating user settings because they changed for profile "{}"'.format(profile_name))
        gdrive.upload_file(**{
            'title': filename,
            'content': filepath,
        ...
        print('SublimeProfiles: User settings are up to date for profile "{}"'.format(profile_name))
...
def upload_local_packages(data):
    ...
    for name in packages:
        ...
        if meta is None:
            ...
            print('SublimeProfiles:  Skipping {} because it is up to date!'.format(name))
            os.remove(filepath)
            continue

        print('SublimeProfiles: Uploading local package because {}'.format(upload_reason))
        gdrive.upload_file(**{
            'title': name,
            'content': filepath,
            'mimeType': 'application/zip',
...
def on_login(result):
    if result is not True:
        content = 'SublimeProfiles: Failed to login, please try again manually using the "SublimeProfiles: Login" command to activate the functionality.'
        ...
        if data is None:
            ...
            print('SublimeProfiles: No profile found, creating a default profile!', data)
            gdrive.upload_file(**{
                'title': 'profiles_info',
                'content': json.dumps(data),
                'mimeType': 'text/json',
            ...
            print('SublimeProfiles: Profiles synced!', data)
        else:
            print('SublimeProfiles: Loaded user profile!')
...
class ProfilesList(sublime_plugin.ApplicationCommand):
    def run(self):
        if gdrive.logged_in() == False:
            content = 'SublimeProfiles: You are not logged in, please try to log in manually using the "SublimeProfiles: Login" command'
            sublime.error_message(content)
        else:
            data = gdrive.get_file_contents('profiles_info')
            data = json.loads(data)
    ...
    try:
        while keep_syncing:
            ...
            if elapsed_time >= sync_time:
                print('SublimeProfiles: syncing!')
                last_time = time.time()
                sync_active_profile()
                print('SublimeProfiles: syncing complete!')
            time.sleep(0.1)
    except Exception as e:
        traceback.print_exc(e)
        print('SublimeProfiles: sync thread exception caught!', e)

    syncing = False
    print('SublimeProfiles: sync thread stopped!')

def update_profile_status():
    profile = get_active_profile()
    for w in sublime.windows():
        for v in w.views():
            v.set_status('user_profile', 'Profile: ' + profile)

def plugin_loaded():
    global sync_thread
    print('SublimeProfiles: sync thread started!')
    sync_thread = threading.Thread(target=sync_active_profile_thread)
    sync_thread.start()
    update_profile_status()

def plugin_unloaded():
    print('SublimeProfiles: plugin_unloaded!')
    global sync_thread
    global keep_syncing
    keep_syncing = False
    while syncing:
        print('SublimeProfiles: waiting for sync thread to settle!')
        time.sleep(0.1)
